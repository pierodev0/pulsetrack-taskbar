...
public sealed class SessionCoordinatorTests : IDisposable
{
    ...
    [Fact]
    public async Task PickApp_CreatesSessionAndStartsRunning()
    {
        await _coordinator.PickAppAsync(App);
    ...
    [Fact]
    public async Task PickApp_StopsPreviousSession()
    {
        await _coordinator.PickAppAsync(App);
        var firstSession = _coordinator.SessionId!.Value;

        await _coordinator.PickAppAsync("Other");
    ...
    [Fact]
    public void Toggle_WithoutApp_ReturnsFalse()
    {
    ...
    [Fact]
    public async Task Stop_ClosesSessionAndBlock()
    {
        await _coordinator.PickAppAsync(App);
        _fg.Current = App;
        _sched.Fire(4);
