...
- **Microsoft.Data.Sqlite 9.0.9** (pineado, no flotante) — DB propia en `%AppData%/PulseTrackTaskbar/sessions.db`, `journal_mode=WAL` + `busy_timeout=5000`
- **Stopwatch libre**: elegir app es **opcional**. `ForegroundTimer.SelectedApp == null` = corre siempre, sin filtrar por foreground; con app seleccionada, solo acumula cuando esa app está al frente
